...
def Api_connect():
    Api_Id="AIzaSyDg2Jp-bD9J08h_NJSkj7r_uj4WX-KTid0"
...
def get_playlist_details(channel_id):
    ...
    while True:
        request=youtube.playlists().list(
            ...
            channelId="UCOKudVNp8FkL9Jl3ix0tU8w",
            maxResults=50,
...
client=pymongo.MongoClient("mongodb+srv://teenalourthan:valanteena@cluster0.15lhdwt.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
db=client["youtube_data"]

def Channel_details(channel_id):
    ch_details=get_channels_info(channel_id)
...
def videos_table(channal_name_s):
...
def comments_table(channel_name_s):
    ...
    df_single_comment_details=pd.DataFrame(single_comments_details)
    


    for index,row in df_single_comment_details.iterrows():
            insert_query='''insert into playlists(comment_Id,
                                                video_Id,
                                                Comment_Text,
                                                Comment_Author,
                                                Comment_published 
...
with st.sidebar:
    st.title(":red[YOUTUBE DATE HAVERSTING AND WAREHOSING]")
